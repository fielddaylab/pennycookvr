    ...
    static public class ScriptDatabaseUtility {
        ...
        static private bool IsValidTrigger(ScriptNode node, ScriptNodeLookupArgs request) {
            ...
            if ((node.Flags & ScriptNodeFlags.HasTimeCooldown) == 0) {
                ...
                if (node.RepeatPeriod.NodeWindow > 0 && request.PersistenceMap.HasRecentlySeen(node.Id(), node.RepeatPeriod.NodeWindow)) {
                    if (DebugFlags.IsFlagSet(ScriptDebugFlags.LogNodeEvaluation)) {
                ...
                if (node.RepeatPeriod.TimeWindow > 0 && request.PersistenceMap.HasRecentlySeenAfterTimestamp(node.Id(), request.CurrentTime - node.RepeatPeriod.TimeWindow)) {
                    if (DebugFlags.IsFlagSet(ScriptDebugFlags.LogNodeEvaluation)) {
